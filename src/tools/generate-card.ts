...
请为以上 ${planData.pages.length} 页生成文案。`;
      ...
      let copyResult: { pages: Array<{
        title: string; subtitle?: string; body?: string;
        bullets?: string[]; caption?: string; imagePrompt?: string;
      ...
      // 3. Generate images — AI text-to-image + web search in parallel
      const imagePromises = planData.pages.map(async (page, i) => {
        ...
        // 3a. AI text-to-image (only for pages with imagePrompt)
        if (imgPrompt) {
          try {
            ...
            // T2I failed, continue with web search only
          }
        }

        // 3b. Web image search (for all pages)
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: JSON.stringify({ pages: copyResult.pages, meta: copyResult.meta, images }, null, 2),
          },
        ],
      };
    }
