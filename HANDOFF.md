...
**未变动**
- Recipe 计数：仍是 M01-M19 + S01-S15 + C01-C12 = 46 个。
...
```bash
cd /Users/guohao/Documents/code/HyperFrames-test/social-card-skill/local-tests/smoke-ai-tools
node render.mjs
...
- 不要改 PPT skill（在 `~/.claude/skills/ppt-skill/`）。本 skill 借了它的视觉原则但**独立维护**。
...
Topic: **Rendering pipeline + Neo-Chinese visual system + image intelligence**.

**New**:
- scripts/render.mjs — standalone concurrent renderer with --parallel N, --board, --device-scale, --no-remote-fonts.
- scripts/image-utils.py — image intelligence (dominant colors, object-position, CSS tint).
- ssets/template-neochinese-card.html — third seed template (scholar/ink/seal/scroll, 5 themes, vertical writing, C01-C12).
- layout-recipes.md new section: Neo-Chinese Recipes C01-C12 (12 recipes).
- 	heme-presets.md new section: Neo-Chinese Palettes (5 themes).

**Modified**:
- HANDOFF.md recipe counts: 34 -> 46 (M01-M19 + S01-S15 + C01-C12).
