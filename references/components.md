...
Both seed templates now enforce professional CJK typesetting via CSS. These rules apply globally across all boards.

### Hanging Punctuation

`hanging-punctuation: first allow-end last` lets CJK punctuation marks (\\u3001\\u3002\\uff0c\\u3002\\uff1b\\uff1a\\u201c\\u201d\\u2018\\u2019\\uff08\\uff09\\u300a\\u300b) bleed slightly outside the text box. This creates optically straight text block edges that look more professional than browser-default justification.

### Line Breaking

`line-break: strict` prevents CJK punctuation from appearing at the start of a line - a typographic taboo in Chinese publishing. Combined with `word-break: keep-all` (implicit in CJK contexts), it produces natural-looking paragraph blocks.

### Inter-Script Spacing

`text-spacing: normal` automatically inserts a 1/4em gap between:
- CJK characters and Latin letters (e.g. "打开Claude" -> "打开 Claude")
- CJK characters and Arabic digits (e.g. "第3步" -> "第 3 步")

This eliminates the need for manual spaces in mixed-script text. The property is set at the `html, body, .content, .poster` level so all text inherits it.

**Display titles override**: `.h-display`, `.h-xl`, `.h-hero`, `.h-statement`, `.num-mega`, `.num-xl`, `.h-md`, and all Editorial serif title classes set `text-spacing: none` because the auto-gap feels too wide at display sizes (88-240px). Titles should rely on explicit `letter-spacing` values instead.

### Browser Compatibility

| Property | Chrome/Edge | Safari | Firefox |
|---|---|---|---|
| `hanging-punctuation` | 123+ | Partial (allow-end only) | Not supported |
| `line-break: strict` | 1+ | 1+ | 1+ |
| `text-spacing` | 123+ | Not supported | Not supported |
| `text-rendering: optimizeLegibility` | 1+ | 1+ | 1+ |

Since Playwright renders via Chromium, all properties are available at render time. If Safari rendering becomes necessary in the future, add a `@supports not (text-spacing: normal)` fallback block that sets `letter-spacing: 0.02em` on body text.

### CJK Font Fallback Chain

Both templates strengthen the CJK sans-serif stack:

- `Noto Sans SC` (primary, covers all Unicode ranges)
- `Source Han Sans SC` (Noto Sans CJK SC, mainland China character forms)
- `HanHei SC` (macOS system HanHei, extensive CJK coverage)
- `PingFang SC` (macOS/iOS system default)
- `Microsoft YaHei UI` (Windows system default)

### Vertical Writing Mode

The seeds do not ship `writing-mode: vertical-rl` support by default. If the Neo-Chinese style system is added in the future, the CJK typography block should be extended with:

- `text-orientation: upright` for CJK glyphs in vertical flow
- `text-combine-upright: digits 2` for 2-digit numbers in vertical text
- `line-break: strict` naturally prevents punctuation at line-start in vertical mode too

### What NOT to Do

- Do not override `text-spacing` on body/lead elements - the per-element override is only for display titles.
- Do not use `word-break: break-all` on Chinese text - it will split CJK words at arbitrary positions.
- Do not use negative `letter-spacing` on CJK body text - Chinese characters need breathing room; negative tracking is only for Latin display.
- Do not set `text-align: justify` on CJK paragraphs without `text-justify: inter-ideograph` - browser-default justification stretches Latin word-spaces, which is wrong for ideographic scripts.
