 ...
 *   R8  CJK-Latin spacing    CJK chars adjacent to Latin/digits without spacing gap    browser-default <figure> margin offsets media alignment
 */
...
for (const s of sections) {
  ...
  // R7 figure margin drift . R8 CJK spacing — catches browser-default 40px figure margins on custom media blocks.
...
﻿  // R8 CJK-Latin spacing -- detect CJK chars directly adjacent to Latin/digit chars
  // without a space in between. Only runs as WARN (not FAIL) since text-spacing
  // CSS property handles this at render time in Chrome 123+.
  const cjkSpacingIssues = await s.evaluate(el => {
    const out = [];
    const checkNodes = el.querySelectorAll('.body, .lead, .h-xl, .h-hero, .h-display, .h-statement, .pullquote, [class*="tutorial-text"], .qa-a, .qa-q, .chat-body');
    const cjkLatin = /[\u4e00-\u9fff\u3400-\u4dbf\uf900-\ufaff][a-zA-Z0-9]|[a-zA-Z0-9][\u4e00-\u9fff\u3400-\u4dbf\uf900-\ufaff]/;
    for (const n of checkNodes) {
      const text = n.textContent || '';
      if (cjkLatin.test(text)) {
        const m = text.match(/[\u4e00-\u9fff\u3400-\u4dbf\uf900-\ufaff][a-zA-Z0-9]|[a-zA-Z0-9][\u4e00-\u9fff\u3400-\u4dbf\uf900-\ufaff]/);
        const ctx = m ? text.slice(Math.max(0, m.index - 8), m.index + m[0].length + 8) : text.slice(0, 30);
        const cls = n.className ? n.className.split(' ')[0] : n.tagName.toLowerCase();
        out.push({ cls, ctx: ctx.trim().slice(0, 40) });
      }
    }
    return out.slice(0, 5);
  });
  for (const c of cjkSpacingIssues) {
    warns.push({ rule: "R8", msg: `."${c.cls}" has CJK-Latin adjacency near "${c.ctx}"`, fix: "add a space between CJK and Latin/digits, or rely on text-spacing CSS (Chrome 123+)" });
  }
