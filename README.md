...
- 🖋 **三套视觉系统**:电子杂志风做氛围与叙事,瑞士风做事实与结构,新中式做东方美学,三套共用同一份工作流
- 📐 **3 个画板尺寸**:`.poster.xhs` 1080×1440(小红书 3:4)、`.poster.wide` 2100×900(公众号 21:9)、`.poster.square` 1080×1080(公众号 1:1)
- 🧩 **46 个版式骨架**:Editorial 19 个(`M01-M19`)+ Swiss 15 个(`S01-S15`)+ Neo-Chinese 12 个(`C01-C12`)
- 🎨 **15 套主题预设**:Editorial 6 套 + Swiss 4 套 + Neo-Chinese 5 套(青瓷、绛红、藤黄、黛紫、月白)
- 🔌 **MCP 服务**:3 个工具(`plan_card`/`generate_card`/`compose_card`),接魔塔社区 LLM + 文生图,部署标准 stdio 协议
- 🖼 **图源工作流**:用户图优先;无图时通过 WebFetch 检索 Unsplash / Pexels / Flickr CC 免 Key 取图
- 🌫 **WebGL 墨流背景**:杂志风 hero 页可挂动态墨流;低性能或截图时可禁用
- 🪧 **图片底图遮罩 + 人脸避让**:满铺图必须叠遮罩,文字落点要避开主体,`references/image-overlay.md` 给硬规则 + 脚本 `scripts/image-utils.py`
...
- 📄 **单文件 HTML + Playwright 渲染**:不需要前端构建链,`node scripts/render.mjs` 直接出 PNG
- 🤖 **MCP 自动编排**:输入提示词 → 自动规划版式 → 调用魔塔 LLM 写文案 → 搜索/生成配图 → 合成完整 HTML
...
```json
{
  "tool": "plan_card",
  "arguments": {
    "prompt": "我想做一个新中式风格的茶文化小红书图文,5 张卡片"
  }
}
```
...
| 主题 | 主色 / 底色 / 强调色 | 适合场景 |
|------|----------------------|---------|
| 🏺 **青瓷 Celadon** | `#2c4a3b` / `#f7f3eb` / `#8ba888` | 茶事、民艺、园林、素食、东方生活 |
| ❤️ **绛红 Crimson** | `#6b1a2a` / `#faf3eb` / `#c4826e` | 节庆、婚嫁、传统工艺、东方建筑 |
| 🌼 **藤黄 Gamboge** | `#7a6a2a` / `#fbf6e8` / `#c4a94c` | 禅修、书法、古籍、文化讲座 |
| 🪻 **黛紫 Dark Iris** | `#3d2e5c` / `#f8f4f0` / `#9b8ec4` | 哲学、诗词、香道、冥想、雅集 |
| 🌙 **月白 Moon White** | `#1a2744` / `#f0f2f5` / `#8ba0c4` | 山水、雪景、宋韵、极简东方
...
├── mcp.json                              ← 魔塔 MCP 广场元数据
├── package.json                          ← MCP 服务依赖与脚本
...
│   ├── template-neochinese-card.html     ← 新中式种子(5 主题 / 3 画板)
│   ├── magazine-bg-webgl.js              ← WebGL 墨流背景
│   └── screenshot-backgrounds/           ← 9 张截图舞台底(WebP)
│       ├── style-a/                      ←   Editorial 5 套
│       └── style-b/                      ←   Swiss 4 套
├── src/                                  ← MCP 服务源代码
│   ├── server.ts                         ← MCP 服务入口,注册 plan_card/generate_card/compose_card
│   ├── tools/
│   │   ├── plan-card.ts                  ← 工具 1:品类路由 + 版式规划
│   │   ├── generate-card.ts              ← 工具 2:LLM 文案 + 文生图 + 图源搜索
│   │   └── compose-card.ts               ← 工具 3:套模板合成完整 HTML
│   ├── services/
│   │   ├── modelscope-llm.ts             ← 魔塔 OpenAI 兼容 API 调用
│   │   ├── modelscope-t2i.ts             ← 魔塔文生图异步 API
│   │   └── web-image-search.ts           ← WebFetch 免 Key 图源搜索(Unsplash/Pexels/Flickr CC)
│   ├── data/
│   │   ├── layout-recipes.ts             ← 46 个版式元数据(M01-M19/S01-S15/C01-C12)
│   │   ├── theme-presets.ts              ← 15 套主题色预设
│   │   └── category-routes.ts            ← 11 个品类路由与版式推荐
│   ├── utils/
│   │   ├── html-builder.ts               ← HTML 合成器(主题/版式/图片注入)
│   │   ├── image-encoder.ts              ← URL → Base64 编码
│   │   └── prompt-builder.ts             ← LLM 提示词构建(含排版硬规则)
│   └── templates/
│       └── loader.ts                     ← 运行时模板加载器
├── scripts/
│   ├── render.mjs                        ← Playwright 并行渲染器
│   └── image-utils.py                    ← 图片智能处理(遮罩/裁剪/调色)
└── references/
    ├── platform-specs.md                 ← 平台 × 分辨率 × 命名
    ├── style-system.md                   ← 三种风格的硬规则与反模式
    ├── theme-presets.md                  ← 15 套色票详解
    ├── layout-recipes.md                 ← 46 个版式骨架(M01-M19 + S01-S15 + C01-C12)
...
- [x] MCP 服务上线:3 个工具 + 魔塔 LLM + 文生图 + 免 Key 图源
- [x] 新中式视觉系统:12 个版式 + 5 套主题
- [x] 渲染管线优化:并行 Playwright 渲染 + 图片智能处理脚本
- [ ] 扩展更多数据可视化版式(折线 / 雷达 / 热力图骨架)
- [ ] AI 出图后处理:渲染后主动询问是否做局部修复 / 整张重出
- [ ] 在 WorkBuddy 等平台提交 MCP 上架
- [ ] 补充更多小红书品类的官方推荐版式包(目前 11 个里 7 个端到端可用)
...
**为什么不允许自定义颜色?**
和 PPT skill 同样的理由——这个 Skill 的核心价值是稳定产出。自由选色会破坏整体风格,只允许从 15 套预设(Celadon 青瓷、Crimson 绛红、IKB 克莱因蓝等)里挑。

**能用其他模型生图吗?**
可以。MCP 服务优先走魔塔 Tongyi-MAI/Z-Image-Turbo,也支持替换为其他 OpenAI 兼容的文生图 API。SKILL.md Step 4 写清了取图协议:用户图 → AI 生图 → 网络取图。
...
- 新增视觉系统时,同步更新 `src/data/layout-recipes.ts`、`src/data/theme-presets.ts`、`src/data/category-routes.ts`
- 新增 MCP 工具时,在 `src/server.ts` 注册 + 在 `mcp.json` 的 tools 列表添加
